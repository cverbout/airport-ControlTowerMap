...
var mymap = L.map('map', {
    ...
    attribution: 'air tower stuff | By Chase Verbout'
  }).addTo(mymap);
...
function setColor(count) {
    var id = 0;
    if (count > 59) { id = 4; }
    else if ( count > 26 && count <= 59) { id = 3; }
    else if (count > 15 && count <= 26) { id = 2; }
    else if (count > 8 &&  count <= 15) { id = 1; }
    else  { id = 0; }
    return colors[id];
}
...
legend.onAdd = function () {
    ...
    div.innerHTML += '<i class="fa fa-plane marker-color-1"></i><p> No Air Traffic Tower </p>';
    div.innerHTML += '<i class="fa fa-plane marker-color-2"></i><p> Air traffic Tower </p>';
